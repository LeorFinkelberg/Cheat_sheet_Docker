...
$ docker pull docker.idp.yc.ziiot.ru/_ds_and_math_users/zyopt:dev
\end{lstlisting}
...
\subsection{Примеры файла docker-compose.yml}

\subsubsection{Автоматизация настройки запуска контейнера}
...
\subsubsection{MongoDB, Redis, фронтэнд, бекэнд и воркер}

\begin{lstlisting}[
title = {\sffamily docker-compose.yaml},
style = bash,
numbers = none
]
version: '3'
...
После окончания работ с контейнерами нужно удалить неиспользуемые тома
\begin{lstlisting}[
style = bash,
numbers = none
]
...
WORKDIR /app
# СОДЕРЖИМОЕ директории ./app на хосте будет скопировано
# в директорию app контейнера
COPY app /app
COPY cmd.sh /
...
Замечание: конструкция \verb|COPY dir /app| означает, что \emph{содержимое} (контент)
директории \verb|dir| хоста будет скопировано в директорию \verb|app| контейнера,
а не сама директория \verb|dir|!
...
<<Обезвредить>> эти бинарные файлы можно командой \verb|chmod a-s|, отменяющей установку бита suid \cite[\strbook{330}]{mouat:docker-2017}
\begin{lstlisting}[
style = bash,
numbers = none
]
...
Dockerfile для запуска ZyOpt в режиме командной строки выглядит так
\begin{lstlisting}[
title = {\sffamily Dockerfile},
style = bash,
numbers = none
...
ARG USER_ID  # билдовый аргумент; используется при сборке образа
ARG GROUP_ID  # билдовый аргумент; используется при сборке образа

ENV CONDA_ENV_NAME zyopt-env

LABEL version="1.0"
LABEL descrioption="Image for local work with heuristic manager ZyOpt CLI"
...
RUN conda env create -n ${CONDA_ENV_NAME} -f environment.yaml

# SHELL ["conda", "run", "-n", "zyopt-env", "/bin/bash", "-c"]
# Чтобы можно использовать переменные окружения, следует SHELL
# заменить на CMD
# https://stackoverflow.com/questions/40454470/how-can-i-use-a-variable-inside-a-dockerfile-cmd
CMD ["/bin/bash", "-c", "conda", "run", "-n", "${CONDA_ENV_NAME}", "/bin/bash", "-c"]

# Install package
RUN conda run --no-capture-output -n ${CONDA_ENV_NAME} python setup.py install
...
# ENTRYPOINT conda run --no-capture-output -n ${CONDA_ENV_NAME} zyopt-cli --help
ENTRYPOINT ["/bin/bash", \  # /bin/bash обязательно на первой строке; переносить нельзя!
            "-c", \
            "exec conda run --no-capture-output -n ${CONDA_ENV_NAME} zyopt-cli \
            --path-to-problem ${PATH_TO_PROBLEM} \
            --solver-name-relax-phase ${SOLVER_NAME_RELAX_PHASE} \
            --solver-name-milp-phase ${SOLVER_NAME_MILP_PHASE} \
            --selected-vars-ratio ${SELECTED_VARS_RATIO:-0.95} \
            --time-budget ${TIME_BUDGET:-600} \
            --path-to-relax-params ${PATH_TO_RELAX_PARAMS} \
            --path-to-milp-params ${PATH_TO_MILP_PARAMS} \
            --path-to-sol ${PATH_TO_SOL} \
            --use-binary ${USE_BINARY:-true} \
            --use-integer ${USE_INTEGER:-true} \
            --use-warm-start ${USE_WARM_START:-true} \
            --use-recommended-seeds ${USE_RECOMMENDED_SEEDS:-true} \
            --random-seeds ${RANDOM_SEEDS:-true} \
            --n-random-seeds ${N_RANDOM_SEEDS:-true} \
            --lb ${LB:-0.0} \
            --ub ${UB:-0.0} \
            --var-ub-shift ${VAR_UB_SHIFT:-0.1}" \
]
\end{lstlisting}
...
Или с помощью обычного текстового файла с переменными окружения
\begin{lstlisting}[
title = {\sffamily cli.env},
style = bash,
numbers = none
]
PATH_TO_PROBLEM=./data/2023_07_YANOS_2690.mps
SOLVER_NAME_RELAX_PHASE=highs
SOLVER_NAME_MILP_PHASE=highs
SELECTED_VARS_RATIO=0.95
TIME_BUDGET=60
PATH_TO_RELAX_PARAMS=./data/highs_relax.set
PATH_TO_MILP_PARAMS=./data/highs_milp.set
PATH_TO_SOL=./data/sols/tmp.sol
RANDOM_SEEDS=42
N_RANDOM_SEEDS=100
USE_BINARY=true
USE_INTEGER=true
USE_WARM_START=true
USE_RECOMMENDED_SEEDS=true
LB=0.0
UB=0.0
VAR_UB_SHIFT=0.1
\end{lstlisting}

А запуск будет выглядеть так
\begin{lstlisting}[
style = bash,
numbers = none
]
$ sudo docker run --rm --env-file ./cli.env -v $(pwd)/data:/data -v $(pwd)/data/sols:/data/sols zyopt-cli
\end{lstlisting}
